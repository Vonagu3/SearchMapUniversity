...
import com.example.searchmapuniversity.utils.*
    ...
    override fun getUniversities(fetchFromRemote: Boolean, query: String?): Flow<Result<List<UniversityInfoItem>>> =
        flow {
            ...
            try {
                ...
                if (loadFromCache) {
                    emit(Result.Success(data = localUniversityInfoList.map {
                        universityInfoConverter.convert(
                            it
                        )
                    }))
                    return@flow
                ...
                val remoteUniversityFileInfo =
                    yandexCloudApi.getAllFiles(token = YANDEX_CLOUD_ACCESS_TOKEN).fileItems
//                .first{ it.path.split("/")[1] == DIR_NAME }
                        .filter { it.path.split("/")[1] == DIR_NAME }
                        .map { universityFileItemConverter.convert(it) }
                        .first()
                val universityFileByteStream =
                    yandexCloudApi.downloadFile(fileUrl = remoteUniversityFileInfo.fileUrl)
                        .byteStream()
                val universityInfoList =
                    universityXlsxParser.parse(stream = universityFileByteStream)
...
//    }
    ...
    override fun likeUniversity(universityInfoItem: UniversityInfoItem): Flow<Result<UniFeedback>> =
        flow {
            try {
                val isLiked = abs(1 - universityInfoItem.like)
                universityDao.likeUniversity(universityInfoItem.name, isLiked)
                emit(
                    Result.Success(
                        if (isLiked == 1) UniFeedback.LIKED
                        else UniFeedback.UNLIKED
                    )
                )
            } catch (e: SQLiteException) {
                emit(Result.Error(DATABASE_ERROR))
